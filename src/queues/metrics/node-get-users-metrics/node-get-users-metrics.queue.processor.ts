...
import { MetricsUser } from '@squidward-node/contracts/shcemas';
...
export class NodeGetUsersMetricsQueueProcessor extends WorkerHost {
    ...
    private async saveMetrics(
        ...
        const aggregated = Object.values(metrics.reduce<Record<string, MetricsUser>>((acc, curr) => {
            if (!acc[curr.usr]) {
                acc[curr.usr] = { ...curr };
            } else {
                acc[curr.usr].up += curr.up;
                acc[curr.usr].down += curr.down;
            }
            return acc;
        }, {}));
        
        for (const part of chunk(aggregated, 500)) {
